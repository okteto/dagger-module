...
import (
	"context"
	"dagger/okteto-dagger-module/internal/dagger"
	"encoding/json"
	"fmt"
	"os"
)
...
func (r OktetoDaggerModule) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *OktetoDaggerModule) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	switch parentName {
	case "OktetoDaggerModule":
		switch fnName {
		case "SetContext":
			var parent OktetoDaggerModule
			...
			return (*OktetoDaggerModule).SetContext(&parent, context, token), nil
		case "PreviewDeploy":
			var parent OktetoDaggerModule
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*OktetoDaggerModule).PreviewDeploy(&parent, ctx, repo, branch, pr, context, token)
		case "PreviewDestroy":
			var parent OktetoDaggerModule
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*OktetoDaggerModule).PreviewDestroy(&parent, ctx, branch, context, token)
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("A generated module for OktetoDaggerModule functions\n\nThis module has been generated via dagger init and serves as a reference to\nbasic module structure as you get started with Dagger.\n\nTwo functions have been pre-created. You can modify, delete, or add to them,\nas needed. They demonstrate usage of arguments and return types using simple\necho and grep commands. The functions can be called from the dagger CLI or\nfrom one of the SDKs.\n\nThe first line in this comment block is a short description line and the\nrest is a long description with more detail on the module's purpose or usage,\nif appropriate. All modules should have a short description.\n").
			WithObject(
				dag.TypeDef().WithObject("OktetoDaggerModule").
					WithFunction(
						dag.Function("SetContext",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Returns a container that has Okteto CLI with the correct context set\nexample usage:\ndagger call set-context --context=cloud.okteto.com --token=$OKTETO_TOKEN").
					...
					WithFunction(
						dag.Function("PreviewDeploy",
							...
							WithDescription("Deploys a preview environment in the specified okteto context\nexample usage:\ndagger call preview-deploy --repo=https://github.com/okteto/todolist-pulumi-s3 --branch=name-change --pr=https://github.com/okteto/todolist-pulumi-s3/pull/2 --context=cloud.okteto.com --token=$OKTETO_TOKEN").
					...
					WithFunction(
						dag.Function("PreviewDestroy",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Destorys a preview environment at the specified okteto context\nexample usage:\ndagger call preview-destroy --branch=name-change --context=cloud.okteto.com --token=$OKTETO_TOKEN").
